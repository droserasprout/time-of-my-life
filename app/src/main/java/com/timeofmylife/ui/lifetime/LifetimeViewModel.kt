...
class LifetimeViewModel(repo: FinanceRepository) : ViewModel() {
    ...
    fun setBudgetMode(mode: BudgetMode) {
        _budgetMode.value = mode
    }

    val rows: StateFlow<List<LifetimeRow>> =
        combine(repo.balances, repo.budgetItems, _budgetMode) { balances, items, mode ->
